...
export function PredictionsEntryClient({ data }: { data: PredictionsPageData }) {
  ...
  const availableStages = useMemo(() => {
    const stages = Array.from(new Set(editableMatches.map((match) => match.stage)));
    return ["all", ...stages];
  }, [editableMatches]);
              ...
              <>
                  ...
                  >
                    <div className="mb-4 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                      <div>
                        <p className="text-sm font-semibold uppercase tracking-[0.24em]" style={{ color: "var(--color-text-subtle)" }}>
                          Clasificados previstos
                        </p>
                        <p className="mt-1 text-sm" style={{ color: "var(--color-text-subtle)" }}>
                          Según tus marcadores de fase de grupos, estos equipos avanzarían desde cada grupo.
                        </p>
                      </div>
                    </div>
                    {predictedGroupStandings.length === 0 ? (
                      <div className="rounded-[1.6rem] border border-white/10 bg-slate-950/80 p-6 text-sm text-slate-400">
                        Introduce pronósticos de la fase de grupos para ver qué equipos serían los más probables en clasificarse.
                      </div>
                    ) : (
                      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                        {predictedGroupStandings.map((groupStanding) => (
                          <div
                            key={groupStanding.group}
                            className="rounded-[1.7rem] border border-white/10 bg-slate-950/80 p-4"
                          >
                            <p className="text-xs uppercase tracking-[0.24em] text-slate-400">{groupStanding.group}</p>
                            <div className="mt-3 space-y-2 text-sm text-slate-100">
                              {groupStanding.qualifiers.map((entry, index) => (
                                <div key={entry.team} className="flex items-center justify-between gap-3 rounded-2xl bg-slate-900/70 px-3 py-3">
                                  <div>
                                    <p className="font-semibold">{index === 0 ? "1.º" : "2.º"}</p>
                                    <p className="text-xs text-slate-400">{entry.team}</p>
                                  </div>
                                  <div className="text-right text-xs text-slate-400">
                                    <p>{entry.points} pts</p>
                                    <p>DG {entry.goalDifference}</p>
                                  </div>
                                </div>
                              ))}
                            </div>
                    ...
                    {predictedBestThirdPlacers.length > 0 && (
                      <div className="mt-6 rounded-[1.7rem] border border-white/10 bg-slate-950/80 p-4">
                        <div className="mb-3 flex items-center justify-between gap-4">
                          <div>
                            <p className="text-xs uppercase tracking-[0.24em] text-slate-400">Mejores terceros</p>
                            <p className="text-sm text-slate-500">Los 8 mejores terceros de todos los grupos según puntos, diferencia de goles y goles anotados.</p>
                          </div>
                        </div>
                        <div className="grid gap-3 sm:grid-cols-2">
                          {predictedBestThirdPlacers.map((entry, index) => (
                            <div key={`${entry.group}-${entry.team}-${index}`} className="flex items-center justify-between gap-3 rounded-2xl bg-slate-900/70 px-3 py-3 text-sm text-white">
                              <div>
                                <p className="font-semibold">{index + 1}. {entry.team}</p>
                                <p className="text-xs text-slate-400">{entry.group} - {entry.points} pts - DG {entry.goalDifference} - GF {entry.goalsFor}</p>
                              </div>
                              <div className="text-right text-xs text-slate-400">
                                <p>{entry.matches} partidos</p>
                              </div>
                            </div>
                          ))}
                        </div>
